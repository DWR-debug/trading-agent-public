...
from automation.trial_013_long_short_leverage_validation_2026_09_24 import _normalized_weights, _portfolio_rows
...
def test_normalized_weights_never_exceed_one_gross_exposure():
    ...
    assert all(sum(abs(value) for value in row.values()) <= 1.0 + 1e-12 for row in weights)


def test_portfolio_rows_have_exact_pit_return_count():
