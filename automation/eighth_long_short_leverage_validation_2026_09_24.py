"""Eighth validation: pre-registered long/short and leverage control.
...
from research.leverage_model import LeverageConfig, simulate_portfolio_path, portfolio_period_return
...
UNIVERSE = "validation_2026_09_24_eighth_leverage"
TARGET_COUNT = 3500
RESEARCH_COUNT = 2798
HOLDOUT_COUNT = 700
FEE_RATE = 0.001
...
def _manifest(path: Path) -> dict:
    ...
    if manifest.get("target_count") != TARGET_COUNT or manifest.get("source") != "yahoo_chart":
        raise ValueError("Unexpected eighth-validation data source/count.")
...
def run_validation(data_dir: Path, manifest_path: Path, output_path: Path) -> dict:
    ...
    report = {
        "schema_version": 1,
        "diagnostic_type": "eighth_long_short_leverage_validation",
        "status": "COMPLETED",
        "source": {
            "universe": UNIVERSE,
