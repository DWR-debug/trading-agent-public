"""Pre-registered Trial 025: monthly market-residual-volatility control.
...
TRIAL_ID = "T-2026-09-24-025"
UNIVERSE = "validation_2026_09_24_idio_volatility_us_stocks"
...
def _load_manifest(path: Path) -> dict:
    ...
    if manifest.get("universe") != UNIVERSE:
        raise ValueError("Unexpected Trial 025 universe.")
    if manifest.get("target_count") != TARGET_COUNT:
        raise ValueError("Unexpected Trial 025 target count.")
    datasets = manifest.get("datasets") or []
    if tuple(item["symbol"] for item in datasets) != SYMBOLS:
        raise ValueError("Manifest symbols do not match Trial 025 preregistration.")
    if not all(item["candle_count"] == TARGET_COUNT for item in datasets):
        raise ValueError("Not every Trial 025 asset has the target candle count.")
    alignment = manifest.get("calendar_alignment", {})
    if alignment.get("mode") != "timestamp_intersection_tail":
        raise ValueError("Trial 025 requires timestamp-intersection alignment.")
    if alignment.get("aligned_candle_count") != TARGET_COUNT:
        raise ValueError("Trial 025 common calendar length is not 3500.")
    safety = manifest.get("safety", {})
    if safety.get("paper_only") is not True or safety.get("live_trading_enabled") is not False:
        raise ValueError("Trial 025 paper-only safety contract missing.")
    return manifest
...
def _load_assets(data_dir: Path, manifest: dict) -> dict[str, list[Candle]]:
    ...
    if len(common_tail) != TARGET_COUNT:
        raise ValueError("Trial 025 common timestamp intersection is too short.")
...
def _segment(values: list[float]) -> dict:
    equity = 1.0
    peak = 1.0
    max_dd = 0.0
    gains = 0.0
...
def _metrics(values: list[float]) -> dict:
    ...
    rolling = [
        _segment(values[boundaries[i] : boundaries[i + 1]])
        for i in range(5)
    ]
    research = _segment(values[:RESEARCH_COUNT])
    holdout = _segment(values[RESEARCH_COUNT : RESEARCH_COUNT + HOLDOUT_COUNT])
    ratio = (
        holdout["return"] / research["return"]
        if research["return"] > 0.0
        else 0.0
    )
    return {
        "research": research,
        "holdout": {
            **holdout,
            "oos_to_research_return_ratio": ratio,
        },
        "rolling_research": rolling,
        "rolling_profitable_window_ratio": (
            sum(item["return"] > 0.0 for item in rolling) / len(rolling)
        ),
    }
...
def _portfolio_returns(
    ...
    *,
    ...
    residual_vol = prior_month_residual_volatility(
        timestamps,
        returns_by_symbol,
    ...
    for month in months:
        values = {
            symbol: residual_vol[symbol][month]
            for symbol in SYMBOLS
            if month in residual_vol[symbol]
        }
        if len(values) != len(SYMBOLS):
            continue
        low = residual_vol_assets(SYMBOLS, values, select_count=SELECT_COUNT)
        high = high_residual_vol_assets(SYMBOLS, values, select_count=SELECT_COUNT)
    ...
    for row in range(TARGET_COUNT - 2):
        ...
        turnover = sum(
            abs(target[symbol] - previous_target[symbol]) for symbol in SYMBOLS
        )
...
def _edge_metrics(edge_by_row: list[float | None]) -> dict:
    if len(edge_by_row) != TARGET_COUNT - 2:
        raise ValueError("Residual-vol edge series must contain 3498 PIT rows.")
    research = [value for value in edge_by_row[:RESEARCH_COUNT] if value is not None]
    holdout = [
        value for value in edge_by_row[
            RESEARCH_COUNT : RESEARCH_COUNT + HOLDOUT_COUNT
        ] if value is not None
    ]
    if not research or not holdout:
        raise ValueError("Residual-vol edge has no observations in one split.")
    return {
        "research_mean_low_residual_vol_minus_high_residual_vol_daily_return":
            sum(research) / len(research),
        "holdout_mean_low_residual_vol_minus_high_residual_vol_daily_return":
            sum(holdout) / len(holdout),
...
def run(data_dir: Path, manifest_path: Path, output_path: Path) -> dict:
    ...
    for name, multiplier in (
        ...
        scenarios[name] = _metrics(values)

    edge = _edge_metrics(edge_by_row)
    checks = {
        ...
        "stress_1_5x_nonnegative": (
            scenarios["stress_1_5x_cost"]["holdout"]["return"] >= 0.0
        ),
        "stress_2x_nonnegative": (
            scenarios["stress_2x_cost"]["holdout"]["return"] >= 0.0
        ),
        "research_low_residual_vol_edge_positive": (
            edge[
                "research_mean_low_residual_vol_minus_high_residual_vol_daily_return"
            ] > 0.0
        ),
        "holdout_low_residual_vol_edge_positive": (
            edge[
                "holdout_mean_low_residual_vol_minus_high_residual_vol_daily_return"
            ] > 0.0
        ),
    ...
    report = {
        ...
        "preregistration": {
            "document": "docs/trial_025_idiosyncratic_volatility_preregistration_2026_09_24.md",
            "strategy_variants": 1,
        ...
        "methodology": {
            "signal": (
                "sample standard deviation of the prior 252 completed "
                "daily residual returns from a leave-one-out equal-weight "
                "market-factor OLS regression"
            ),
            "market_factor": "equal-weight daily return of the other seven assets",
            "residualization": "OLS with intercept",
            "direction": "long four lowest residual-volatility assets",
...
def main() -> int:
    ...
    print("TRIAL_025_STATUS:", report["status"])
    print("TRIAL_025_DECISION:", json.dumps(report["decision"], sort_keys=True))
    print("TRIAL_025_REPORT_FINGERPRINT:", report["report_fingerprint"])
    return 0
