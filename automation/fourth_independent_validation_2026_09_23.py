"""Fourth independent validation of the fixed 50/50 + 10% vol-budget candidate.

The architecture, research/holdout split, cost scenarios and gates are unchanged.
Only the pre-registered, fully symbol-disjoint ETF universes differ.
...
def _assert_global_disjoint() -> None:
    ...
    expected_size = len(get_universe(TREND_UNIVERSE).symbols) + len(
        get_universe(CS_UNIVERSE).symbols
    )
    if len(new_symbols) != expected_size:
        raise ValueError("Vierte Validierungsuniversen sind intern nicht disjunkt.")
    ...
    if overlap:
        raise ValueError(
            "Vierte Validierungsuniversen überschneiden bestehende Research-Universen: "
            + ", ".join(overlap)
        )
...
def run_validation(
    ...
    report = {
        "schema_version": 1,
        "diagnostic_type": "fourth_independent_candidate_validation_2026_09_23",
        "status": "COMPLETED",
        "candidate_status": (
        ...
        "preregistration": {
            "document": "docs/fourth_independent_validation_2026_09_23_preregistration.md",
...
def main() -> int:
    ...
    print("FOURTH_INDEPENDENT_VALIDATION_STATUS:", report["status"])
