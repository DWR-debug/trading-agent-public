...
UNIVERSE = "validation_2026_09_26_h06_formal_performance"
...
def run(
    *,
    ...
    "research/runs/wide_search/h06_formal_performance_coverage.json",
) -> dict:
    if (
    ...
    for existing in list_universes():
        ...
        if overlap:
            raise RuntimeError(
                f"Formal performance universe overlaps existing universe {existing.name}: {sorted(overlap)}"
            )
    ...
    if not output.is_absolute():
        output = ROOT / output
    snapshot_dir = output.parent / "h06_formal_performance_datasets"
    ...
    result = {
        ...
        "universe": UNIVERSE,
        "purpose": "FORMAL_PERFORMANCE_COVERAGE",
