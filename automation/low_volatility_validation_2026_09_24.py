"""Pre-registered Trial 024: monthly low-realized-volatility cross-sectional control.
...
from research.fixed_low_volatility import daily_close_returns, high_vol_assets, low_vol_assets, prior_month_low_vol

TRIAL_ID = "T-2026-09-24-024"
UNIVERSE = "validation_2026_09_24_low_volatility_us_stocks"
...
def _load_manifest(path: Path) -> dict:
    ...
    if manifest.get("universe") != UNIVERSE:
        raise ValueError("Unexpected Trial 024 universe.")
    if manifest.get("target_count") != TARGET_COUNT:
        raise ValueError("Unexpected Trial 024 target count.")
    datasets = manifest.get("datasets") or []
    if tuple(item["symbol"] for item in datasets) != SYMBOLS:
        raise ValueError("Manifest symbols do not match Trial 024 preregistration.")
    if not all(item["candle_count"] == TARGET_COUNT for item in datasets):
        raise ValueError("Not every Trial 024 asset has the target candle count.")
    alignment = manifest.get("calendar_alignment", {})
    if alignment.get("mode") != "timestamp_intersection_tail":
        raise ValueError("Trial 024 requires timestamp-intersection alignment.")
    if alignment.get("aligned_candle_count") != TARGET_COUNT:
        raise ValueError("Trial 024 common calendar length is not 3500.")
    safety = manifest.get("safety", {})
    if safety.get("paper_only") is not True or safety.get("live_trading_enabled") is not False:
        raise ValueError("Trial 024 paper-only safety contract missing.")
    return manifest
...
def _load_assets(data_dir: Path, manifest: dict) -> dict[str, list[Candle]]:
    ...
    if len(common_tail) != TARGET_COUNT:
        raise ValueError("Trial 024 common timestamp intersection is too short.")
...
def _metrics(values: list[float], split: int) -> dict:
    ...
    rolling = [segment(values[boundaries[i]:boundaries[i + 1]]) for i in range(5)]
    research = segment(values[:split])
    holdout = segment(values[split:split + HOLDOUT_COUNT])
    ratio = holdout["return"] / research["return"] if research["return"] > 0.0 else 0.0
    return {
        "research": research,
        "holdout": {**holdout, "oos_to_research_return_ratio": ratio},
        "rolling_research": rolling,
        "rolling_profitable_window_ratio": sum(item["return"] > 0.0 for item in rolling) / len(rolling),
    }
...
def _portfolio_returns(
    ...
    *,
    ...
    for month in months:
        values = {
            symbol: vol_by_symbol[symbol][month]
            for symbol in SYMBOLS
            if month in vol_by_symbol[symbol]
        }
        if len(values) != len(SYMBOLS):
            continue
        low = low_vol_assets(SYMBOLS, values, select_count=SELECT_COUNT)
        high = high_vol_assets(SYMBOLS, values, select_count=SELECT_COUNT)
    ...
    for row in range(TARGET_COUNT - 2):
        ...
        if selected is None:
            ...
            realized = {
                symbol: assets[symbol][row + 2].close / assets[symbol][row + 1].close - 1.0
                for symbol in SYMBOLS
            }
        ...
        portfolio_returns.append(gross_return - BASE_COST * cost_multiplier * turnover)
...
def _edge_metrics(edge_by_row: list[float | None], split: int) -> dict:
    if len(edge_by_row) != TARGET_COUNT - 2:
        raise ValueError("Low-vol edge series length must equal the 3498 PIT return rows.")
    research = [value for value in edge_by_row[:split] if value is not None]
    holdout = [value for value in edge_by_row[split:split + HOLDOUT_COUNT] if value is not None]
    if not research or not holdout:
        raise ValueError("Low-vol edge has no observations in one split.")
    return {
        "research_mean_low_vol_minus_high_vol_daily_return": sum(research) / len(research),
        "holdout_mean_low_vol_minus_high_vol_daily_return": sum(holdout) / len(holdout),
        "research_edge_observation_count": len(research),
        "holdout_edge_observation_count": len(holdout),
    }
...
def run(data_dir: Path, manifest_path: Path, output_path: Path) -> dict:
    ...
    base_turnover = 0.0
    for name, multiplier in (("base", 1.0), ("stress_1_5x_cost", 1.5), ("stress_2x_cost", 2.0)):
        values, current_edge, selections, turnover = _portfolio_returns(
            assets, cost_multiplier=multiplier
        )
        if not edge_by_row:
    ...
    checks = {
        ...
        "research_profit_factor": scenarios["base"]["research"]["profit_factor"] is not None and scenarios["base"]["research"]["profit_factor"] >= 1.10,
        ...
        "holdout_profit_factor": scenarios["base"]["holdout"]["profit_factor"] is not None and scenarios["base"]["holdout"]["profit_factor"] >= 1.10,
        ...
        "research_low_vol_edge_positive": edge["research_mean_low_vol_minus_high_vol_daily_return"] > 0.0,
        "holdout_low_vol_edge_positive": edge["holdout_mean_low_vol_minus_high_vol_daily_return"] > 0.0,
    }
    report = {
        "schema_version": 1,
        "trial_id": TRIAL_ID,
        ...
        "preregistration": {
            "document": "docs/trial_024_low_volatility_preregistration_2026_09_24.md",
            "strategy_variants": 1,
        ...
        "methodology": {
            "signal": "standard deviation of the prior 252 completed daily close-to-close returns at each calendar-month transition",
            "direction": "long four lowest-volatility assets",
        ...
        "base_total_turnover": base_turnover,
        "edge_measure": edge,
        "scenarios": scenarios,
        "decision": {
            "checks": checks,
...
def main() -> int:
    ...
    print("LOW_VOL_STATUS:", report["status"])
    print("LOW_VOL_DECISION:", json.dumps(report["decision"], sort_keys=True))
    print("LOW_VOL_REPORT_FINGERPRINT:", report["report_fingerprint"])
    return 0
