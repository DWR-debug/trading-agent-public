"""Trial 027: fixed long-only TSM 63/126/252 trend-sleeve replacement.
...
TRIAL_ID="T-2026-09-24-027"
UNIVERSE="validation_2026_09_24_tsm_ensemble_candidate"
TREND_SYMBOLS=("IVV","ITOT","SCHX","SCHF","SPAB","IEI","MBB","VCSH")
CS_SYMBOLS=("IYJ","IYC","IYM","IYK","IYW")
...
def _manifest(path):
    ...
    if data.get("universe")!=UNIVERSE or symbols!=tuple(universe.symbols):
        raise ValueError("Manifest passt nicht zum Trial-027-Universum.")
    if data.get("target_count")!=TARGET_COUNT or data.get("source")!="yahoo_chart":
        raise ValueError("Unerwartete Trial-027-Datenbasis.")
...
def _assets(data_dir,manifest):
    ...
    if set(out)!=set(PORTFOLIO_SYMBOLS):
        raise ValueError("Trial-027-Symbole fehlen.")
...
def _tsm_long_flat_signal(closes,index):
    ...
    for lb in TSM_LOOKBACKS:
        change=closes[index]/closes[index-lb]-1.0
        if change>0: pos+=1
        elif change<0: neg+=1
...
def run_validation(data_dir,manifest_path,output_path):
    ...
    report={"schema_version":1,"trial_id":TRIAL_ID,"status":"COMPLETED","research_only":True,
            ...
            "hypothesis":"Replace only the fixed SMA 50/200 long-only trend signal with fixed long-only majority-vote TSM 63/126/252 while preserving the CS sleeve, 10% volatility budget, costs and PIT execution.",
            "source":{"universe":UNIVERSE,"trend_symbols":list(TREND_SYMBOLS),"cross_sectional_symbols":list(CS_SYMBOLS),"portfolio_symbols":list(PORTFOLIO_SYMBOLS),
                     ...
                     "fully_symbol_disjoint":True,"signal_variant_count":1,"manifest_fingerprint":manifest["manifest_fingerprint"]},
            "methodology":{"baseline":"fixed 50/50 SMA 50/200 trend sleeve + 12-1 CS Top-2 + 10% volatility budget",
                           "intervention":"replace only trend signal with long-only majority vote of 63/126/252 session returns",
...
if __name__=="__main__":
    ...
    print("TRIAL_027_STATUS:",r["status"]); print("TRIAL_027_CANDIDATE_STATUS:",r["candidate_status"]); print("TRIAL_027_REPORT_FINGERPRINT:",r["report_fingerprint"]); print("TRIAL_027_CHECKS:",json.dumps(r["gate_contract"],sort_keys=True))
