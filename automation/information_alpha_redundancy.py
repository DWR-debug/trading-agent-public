...
def analyze_redundancy_base(
    ...
    *,
    ...
    """Apply the frozen Q013 analysis to an already-built discovery base."""
    ...
    result = {
        "schema_version": "1.0", "task_id": "Q-013-INFORMATION-ALPHA-MECHANISM-REDUNDANCY-DIAGNOSTIC",
        "status": "MECHANISM_REDUNDANCY_DIAGNOSTIC_ONLY", "research_only": True,
        "start": start.isoformat(), "end": end.isoformat(), "assets": list(DEFAULT_ASSETS),
        ...
        "observations": {"total": len(observations), "event_windows": len(event_observations), "no_event_windows": len(observations) - len(event_observations)},
        "data_quality": base["data_quality"],
        "feature_redundancy": {"all_common_observations_spearman": all_matrix, "event_only_spearman": event_matrix, "group_summary_all": _group_summary(all_matrix), "group_summary_event_only": _group_summary(event_matrix)},
        "return_associations": {"all_common_observations": _return_associations(observations, DEFAULT_ASSETS), "event_only": _return_associations(event_observations, DEFAULT_ASSETS)},
        "selection_used": False, "holdout_used": False, "parameter_search_used": False,
        "feature_selection_used": False, "asset_selection_used": False, "performance_trial_authorized": False,
    ...
    (root / "q013_redundancy.json").write_text(json.dumps(result, indent=2, ensure_ascii=False, allow_nan=False) + "\n", encoding="utf-8")
    return result
...
def run_redundancy_diagnostic(
    ...
    *, output_dir: str | Path = "research/runs/information_alpha_redundancy/q013",
    ...
    base = run_discovery(start, end, assets=DEFAULT_ASSETS, output_path=root / "base_discovery.json")
    return analyze_redundancy_base(base, start, end, output_dir=root)
