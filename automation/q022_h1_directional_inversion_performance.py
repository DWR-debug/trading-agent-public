"""Fixed-rule Q022-H1 Treasury-auction directional-inversion performance runner.
...
TRIAL_ID = "T-2026-09-26-047-PERFORMANCE"
UNIVERSE = "validation_2026_09_26_treasury_auction_directional_inversion"
SYMBOLS = ("MO","PM","GILD","VRTX","ISRG","ZBH","ELV","CI","JCI","FAST","PAYX","CTSH")
TARGET_CANDLES = 3500
...
def run(manifest_path: Path, data_dir: Path, signal_contract_path: Path, authorization_path: Path, output_path: Path) -> dict:
    ...
    if auth.get("trial_id") != TRIAL_ID or auth.get("performance_execution_authorized") is not True:
        raise RuntimeError("Q022-H1 performance authorization missing")
    if auth.get("coverage_artifact_id", 0) <= 0 or auth.get("coverage_workflow_run_id", 0) <= 0:
        raise RuntimeError("Authorization is not bound to a validated coverage artifact")
    if auth.get("execution_scope") != "PERFORMANCE":
        raise RuntimeError("Q022-H1 execution scope mismatch")
    ...
    result = {
        "schema_version": "1.0",
        "trial_id": TRIAL_ID,
        "parent_trial_id": "T-2026-09-26-046R1-PERFORMANCE",
        "intervention": "exact_final_direction_inversion",
        "status": "ALL_GATES_PASSED" if _gates(scenarios)["all_absolute_passed"] else "NO_PROMOTION_EVIDENCE",
        "research_family": "official_event_alpha_directional_inversion",
    ...
    print(json.dumps({"trial_id": TRIAL_ID, "status": result["status"], "result_fingerprint": result["result_fingerprint"], "intervention": "exact_final_direction_inversion"}, sort_keys=True))
    return result
